...
- `free_transfers_if_roll` / `free_transfers_if_transfer` are **next-GW** FT balances after the usual +1 accrual — not “FT left today”. When `free_transfers` is 0, both are often 1; that does **not** mean a transfer is free now.
- Always read `transfer_decision.hit_points_if_transfer`. If it is > 0 (typical when `free_transfers` is 0), any transfer move **must** state the −N hit and the **net** this-week points after that hit (gross delta − hit). Never quote only the gross (+2.2) as if it were free.
- If `transfer_decision.action` is `roll`, Do this must lead with `move_type=hold`. Do **not** also emit a competing `move_type=transfer` for the same swap, and do not add a lineup “Start {in}” for a rejected buy. Explain the rejected alternative only inside the hold `why`, with the hit and net.
...
- If a `transfer_plans` row has `hit_cost > 0` and positive `net_gw_xp`, you may recommend the hit only when `transfer_decision.action` is `transfer` and that net edge is clear. Never invent a -4 that is not in `transfer_plans` / `hit_points_if_transfer`.
...
- If `transfer_decision.action` is `transfer` and `transfer_candidates` is non-empty and news does not veto the buy, prefer `plan_action=revise` with one concrete `move_type=transfer` citing both out_id and in_id from `best_affordable` (or the news-replacement candidate). If `action` is `roll`, do not turn `best_affordable` into a transfer move.
...
- `detail`: 80–150 words. Lead with projected points (who scores more this week and why the XI ranks that way). Mention fixtures only as context. Use start% only when a player is unlikely to play. Explicitly address confusing cases (e.g. benching someone with an easier-looking fixture because their projected points are lower). Put model numbers in parentheses at the end. Avoid jargon.
- `suggested_moves`: at most a few concrete, legal ideas referencing supplied player_ids only.
- Every `suggested_moves` item **must** include a non-empty `why` that leads with points, then optional start%/fixture context, then brackets for totals.
